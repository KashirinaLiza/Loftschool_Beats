...
//помимо открытия попапа через атрибут data-open мы можем просто воспользоваться методом открытия нужно нам попапа, например popup
const buttonOpenPoupupSuccess = document.querySelector('.hamburger--popup-success');
buttonOpenPoupupSuccess.addEventListener('click',() => popupSuccess.open())
